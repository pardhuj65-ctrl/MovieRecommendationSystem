...
def md(content, **kwargs):
    ...
    if isinstance(content, str):
        ...
        # Streamlit's Markdown parser treats indented lines after blank
        # lines inside a <div> as Markdown code blocks. Remove blank lines
        # from HTML snippets so every line stays inside the HTML block.
        if "<div" in content or "<style" in content or "<span" in content:
            content = "\n".join(
                line for line in content.splitlines() if line.strip()
            )

    # Some calls already pass unsafe_allow_html=True. Remove it here so
    # the helper never sends the same keyword twice to Streamlit.
...
# TMDB CACHE
# ============================================================
# Streamlit reruns the script whenever a widget changes. These cached
# wrappers prevent repeated TMDB requests and make the app much faster.
...
with st.sidebar:
    ...
    md(
        '<div class="sidebar-subtitle">Your Smart Movie Companion</div>',
        unsafe_allow_html=True,
    )
...
md(
    '<div class="section-title">🔎 Search Any Movie</div>',
    unsafe_allow_html=True,
)
...
if st.session_state.search_results:
    md(
        '<div class="section-title">🎞️ Search Results</div>',
        unsafe_allow_html=True,
    )
...
""",
    ...
    md(
        '<div class="section-title">💬 Movie Reviews</div>',
        unsafe_allow_html=True,
    )
...
""",
    ...
    md(
        f"""
<div class="section-description">
    Recommendations are based on the selected movie
    and its language.
</div>
""",
        unsafe_allow_html=True,
...
""",
        ...
        md(
            '<div class="info-box">'
            '🎯 No same-language recommendations were found for this movie.'
            '</div>',
            unsafe_allow_html=True,
        )
...
with st.spinner(
    ...
    try:
        latest_movies = cached_latest_movies(
            language_code
        )
    except Exception:
        latest_movies = []
...
with st.spinner(
    ...
    try:
        trending_movies = cached_trending_movies(
            language_code
        )
    except Exception:
        trending_movies = []

# Fallback if TMDB trending has no movies
if not trending_movies:
    try:
        trending_movies = cached_popular_movies(
            language_code
        )
    except Exception:
        trending_movies = []
...
with st.spinner(
    ...
    try:
        popular_movies = cached_popular_movies(
            language_code
        )
    except Exception:
        popular_movies = []
